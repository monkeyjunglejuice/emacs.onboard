...
;; . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
;;; - Minicomp code completion

;; Alternative to the *Completions* buffer; it shows the completion candidates
;; in the minibuffer, e.g. using `fido-vertical-mode' or other `completing-read'
;; UIs. Enable/disable with "M-x eon-minicomp-mode".

(defgroup eon-minicomp nil
  "Minibuffer frontend for in-region completion."
  :group 'eon)

(defcustom eon-minicomp-require-match t
  "If non-nil, require a real candidate in `completing-read'."
  :group 'eon-minicomp
  :type 'boolean)

(defvar eon-minicomp--saved-in-region
  (default-value 'completion-in-region-function)
  "Saved default in-region frontend for fallback.")

(defvar eon-minicomp--saved-recursive nil
  "Saved value of `enable-recursive-minibuffers'.")

(defun eon-minicomp--wrap (buf fn)
  "Return FN wrapped to run in BUF. Return nil if FN is nil."
  (when fn
    (lambda (&rest args)
      (with-current-buffer buf (apply fn args)))))

(defun eon-minicomp--wrap-table (buf table)
  "Return TABLE wrapped to run in BUF when it's a function."
  (if (functionp table) (eon-minicomp--wrap buf table) table))

(defun eon-minicomp--props (buf md)
  "Build `completion-extra-properties' from MD, wrapped for BUF."
  (let ((plist nil))
    (dolist (key '(:annotation-function :affixation-function
                                        :display-sort-function
                                        :cycle-sort-function
                                        :group-function))
      (when-let* ((fn (pcase key
                        (:annotation-function
                         (completion-metadata-get md 'annotation-function))
                        (:affixation-function
                         (completion-metadata-get md 'affixation-function))
                        (:display-sort-function
                         (completion-metadata-get md
                                                  'display-sort-function))
                        (:cycle-sort-function
                         (completion-metadata-get md 'cycle-sort-function))
                        (:group-function
                         (completion-metadata-get md 'group-function)))))
        (setq plist (plist-put plist key (eon-minicomp--wrap buf fn)))))
    (when-let ((cat (completion-metadata-get md 'category)))
      (setq plist (plist-put plist :category cat)))
    plist))

(defun eon-minicomp--minibuffer (beg end collection &optional pred)
  "Minibuffer UI for `completion-in-region'."
  (let* ((origin (current-buffer))
         (initial (buffer-substring-no-properties beg end))
         ;; Compute MD before entering the minibuffer
         (md (completion-metadata initial collection pred))
         (exitf (completion-metadata-get md 'exit-function))
         (collection* (eon-minicomp--wrap-table origin collection))
         (completion-auto-help nil)
         (completion-extra-properties
          (eon-minicomp--props origin md))
         (choice (completing-read "Complete: " collection* pred
                                  eon-minicomp-require-match
                                  initial)))
    (unless (equal choice initial)
      (delete-region beg end)
      (insert choice))
    (when exitf
      (funcall (eon-minicomp--wrap origin exitf) choice 'finished))
    t))

(defun eon-minicomp--dispatch (beg end collection &optional pred)
  "Dispatcher for `completion-in-region'."
  (let* ((local (and (local-variable-p 'completion-in-region-function)
                     (not (eq completion-in-region-function
                              #'eon-minicomp--dispatch))
...
;; Keep recursive minibuffers on while the mode is enabled

(defvar eon-minicomp--guard nil)

(defun eon-minicomp--watch (_sym new _op _where)
  "Keep `enable-recursive-minibuffers' non-nil while mode is active."
  (when (and eon-minicomp-mode (not eon-minicomp--guard) (not new))
    (let ((eon-minicomp--guard t))
      (setopt enable-recursive-minibuffers t))))

(define-minor-mode eon-minicomp-mode
  "Alternative to the *Completions* buffer.

It works when either `fido-vertical-mode', `fido-mode' or Vertico is enabled.
The minor mode sets `completion-in-region-function' to a dispatcher that
uses a minibuffer UI by default, respects buffer-local frontends, and
forces `enable-recursive-minibuffers' while active."
  :group 'eon-minicomp
  :global t
  (if eon-minicomp-mode
      (progn
        (unless (eq (default-value 'completion-in-region-function)
                    #'eon-minicomp--dispatch)
          (setq eon-minicomp--saved-in-region
                (default-value 'completion-in-region-function)))
        (setq-default completion-in-region-function
                      #'eon-minicomp--dispatch)
        (setq eon-minicomp--saved-recursive
              enable-recursive-minibuffers)
        (setopt enable-recursive-minibuffers t)
        (add-variable-watcher 'enable-recursive-minibuffers
                              #'eon-minicomp--watch))
    (setq-default completion-in-region-function
                  eon-minicomp--saved-in-region)
    (remove-variable-watcher 'enable-recursive-minibuffers
                             #'eon-minicomp--watch)
    (when (boundp 'eon-minicomp--saved-recursive)
      (setopt enable-recursive-minibuffers
              eon-minicomp--saved-recursive))))
