* 🚀 Emacs is Like an Ancient Starship From an Advanced Alien Civilization, Found Half-buried in The Amazon Jungle.
...
- Ergonomic leader key / Localleader key menus (super easy to extend).
  ...
  - Graphical Emacs: the leader key is =C-,=; reach the local leader via =C-, C-,=.
  - Emacs in the terminal: reach the leader menu via =C-z=, and the local leader via =C-z C-z=.
...
- Comprehensive [[https://orgmode.org][Org mode]] setup to get started.
