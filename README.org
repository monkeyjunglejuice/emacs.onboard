* 🚀 Emacs is Like an Ancient Starship Built by an Advanced Alien Civilization, Discovered Half-buried in The Jungle.

** This is Your Launch Sequence.

*Emacs onboarding made easy.* Don't want a complex & opaque Emacs distro like Doom Emacs or Spacemacs, but rather a solid foundation to roll your own Emacs config? Or you don't want to configure Emacs at all? But ... just use it? Right now? 🤯

[[file:misc/emacs-onboard.jpg]]

*Single-file Emacs starter kit without 3rd-party packages.*
Almost vanilla Emacs, with just the right amount of sweetness to flatten the learning curve. Unlike others, it's not supposed to replace your ~init.el~, but loaded from there as a library.
...
#+begin_src shell
git clone https://github.com/monkeyjunglejuice/emacs.onboard.git ~/.emacs.onboard --depth 1
#+end_src

*2. Put this in your Emacs init file* directly after the ~(custom-set-faces ... )~ form
...
/You don't have an init file or don't know where it is? With a freshly installed Emacs there is no init file. But you can ask Emacs where it expects to find it:/
...
- macOS
  ...
  - =/Users/USERNAME/.emacs.d/init.el=
- Windows
...
- Leader key & local leader key bindings (easy to extend/change).
  This gives you quasi-infinite room for your own keybindings under the leader prefix.
  ...
  - Works with Evil mode (Vim keybindings etc. in Emacs) -- [[https://github.com/monkeyjunglejuice/emacs.ontop/blob/c1f7fe28df28cfa9323a4da1b6b5a5d0c64477fb/eon-evil.el][example config here]]; e.g. the leader key could be just =,= and the local leader =, ,=; or =SPC= as the leader and =SPC SPC= as the local leader -- or something else.
  - Three options for leader keybindings (customizable via GUI):
    1. Use the pre-configured leader keymap with many useful bindings.
...
- Tree-sitter grammar registry: handle language grammars and trigger build/install declaratively via ~eon-treesitter-ensure-grammar~.
- Smooth pixel-based scrolling (Emacs doesn't do that by default).
- Basic [[https://orgmode.org][Org mode]] setup to get started.
- Garbage collector tuning: includes the "[[The Emacs Garbage Collection Magic Hack][Emacs Garbage Collection Magic Hack]]".
...
- Visit any file when the file name is under the cursor: =<leader> f .=.
...
- Visit any URL in your browser via =<leader> g w= (built-in browser) and =<leader> g W= (system default web browser).
...
- *Modern:* Emacs Lisp code should be written in contemporary style and be free from errors and warnings.
...
- *Reasonable backwards-compatibility:* Don't overdo -- follows the [[https://packages.debian.org/stable/editors/emacs][Emacs version in Debian Stable]], which is a fairly conservative GNU/Linux distro (currently Emacs 30.1) and therefore a good baseline.
...
=M-x= -- Press <Alt>+<x> to access all commands (a.k.a "interactive" functions).
...
- =M-x eon-customize-group= all Emacs ONboard specific preferences in one place.
...
Well, the only problem (as I see it) is: Emacs is tied to the implementation of its underlying Lisp engine right now, which is essentially still single-threaded and not that performant for advanced applications (well, there are some tricks). But when it comes to what can be expressed as text in the widest sense (and most programming is text-based), there's nothing else that comes even close.

* Credits
Emacs ONBOARD has grown over the years, influenced by hundereds of dotemacs configs and blog posts. As it wasn't intended as a public endeavor from the beginning, it's now impossible to trace back what originated where; so you might encounter code that you've seen somwhere else before. Recently adopted some ideas from [[https://github.com/jamescherti/minimal-emacs.d][Minimal Emacs]] (mostly tweaking details) and [[https://github.com/doomemacs/doomemacs][Doom Emacs]].
