...
- Vertical fuzzy completion for commands, files, etc. in the minibuffer (built-in fido-mode).
- Tree-sitter: handle language/grammars and trigger build/install declaratively.
- Toggle between your light/dark theme via =<leader> x t=; and run own Elisp code triggered by switching themes, e.g. for adjusting fonts and other personal styling, depending on the theme variant.
